...
export async function getDataOptions(dataOptions: any[], username: string): Promise<string> {
    let dataBoxes = ''
    ...
    function addDataBox(
        ...
        dataBoxes += `<g id="${name}" transform="translate(${(dataOptions.length - 1 - index) * -108
            ...
            } 24)" fill="${color2}" font-size="16" font-family="Roboto-Regular, Roboto, sans-serif">
                    <tspan x="0" y="0">${count}</tspan>
                </text>
                ${name !== 'commits' && name !== 'contributions'
                ? `<path id="${name}-icon" transform="translate(-71 ${name === 'stars' ? '8' : '6'
                })" fill="${color2}" d="${svg}"/>`
                : svg
            }
            </g>`
...
export async function generateUserActivity(username: string, maxEvents?: number, token?: string): Promise<string[] | undefined> {
    ...
    while (result.length < maxEvents && page <= 3) {
        ...
        const response = await axios.get(`https://api.github.com/users/${username}/events?per_page=100&page=${page}`, { headers: headers });
